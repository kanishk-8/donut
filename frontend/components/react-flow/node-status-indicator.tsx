...
export const SpinnerLoadingIndicator = ({
    ...
    return (
        <div className="relative">
            <StatusBorder className="border-ring/40">{children}</StatusBorder>

            <div className="absolute inset-0 z-50 rounded-[7px] bg-background/50 backdrop-blur-sm" />
            <div className="absolute inset-0 z-50">
                <span className="absolute left-[calc(50%-1.25rem)] top-[calc(50%-1.25rem)] inline-block h-10 w-10 animate-ping rounded-full bg-ring/20" />

                <LoaderCircle className="absolute left-[calc(50%-0.75rem)] top-[calc(50%-0.75rem)] size-6 animate-spin text-ring" />
            </div>
        </div>
    );
};
...
export const BorderLoadingIndicator = ({
    ...
    return (
      ...
      `}
                ...
                >
                    <div className="spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,color-mix(in_oklab,var(--ring)_50%,transparent)_0deg,transparent_360deg)]" />
                </div>
            </div>
            {children}
        </>
